...
function CategoryDialog({
  ...
  useEffect(() => {
    if (open) {
      setForm({ name: editing?.name ?? '', description: editing?.description ?? '', store: '' });
    }
  }, [open, editing]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        >
          ...
          {!isEditing && (
            <div className="space-y-1.5">
              <Label htmlFor="new-category-store">Store *</Label>
              <select
                id="new-category-store"
                required
                value={form.store}
...
export function CategoriesPage() {
  ...
  const { data: categories = [], isLoading } = useQuery<Category[]>({
    queryKey: ['categories', storefront.store],
    queryFn: async () => (await api.get('/categories', { params: storefront })).data,
  });
