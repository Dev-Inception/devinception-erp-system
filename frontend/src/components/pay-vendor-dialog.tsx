...
/**
 * Pays down a vendor's overall AP balance (not scoped to any one stock
 * receipt or sale) — the generic counterpart to
 * RecordVendorPaymentDialog, which only settles one stock receipt.
 */
export function PayVendorDialog({
  vendor,
  open,
  onOpenChange,
}: {
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      return (
        await api.post('/finance/payments/vendor', {
          vendor: vendor!.id,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>{t('Pay Vendor')}</DialogTitle>
          ...
          >
            ...
            <div className="rounded-lg border bg-muted/20 p-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatCurrency(vendor.outstanding)}</span>
              </div>
            </div>
