...
const createSaleValidator = [
  ...
  body("items.*.unitPrice").optional({ values: "falsy" }).isFloat({ min: 0 }).withMessage("Unit price must be non-negative"),
  ...
  body("payment.cash").optional({ values: "falsy" }).isFloat({ min: 0 }).withMessage("Cash must be non-negative"),
  body("payment.online").optional({ values: "falsy" }).isFloat({ min: 0 }).withMessage("Online must be non-negative"),
  body("payment.bankAccount").optional({ values: "falsy" }).isMongoId().withMessage("Invalid bank account"),
  body("payment.receiptRef").optional({ values: "falsy" }).isString().trim().isLength({ max: 500 }).withMessage("Invalid transfer receipt reference"),
];
