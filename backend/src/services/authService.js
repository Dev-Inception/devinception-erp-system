...
async function logout({ accessToken, refreshToken }) {
  ...
  for (const [token, verify] of [
    [accessToken, tokenService.verifyAccessToken],
    [refreshToken, tokenService.verifyRefreshToken],
  ]) {
  ...
  if (payload?.sub) await User.increment('tokenVersion', { by: 1, where: { id: payload.sub } });
}
