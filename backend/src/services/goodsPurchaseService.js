...
const { GoodsPurchase, GoodsPurchaseItem, Vendor, Product, BankAccount, Warehouse } =
  initializeModels();

/**
 * Goods purchase flow. Validates everything up front, then: raises stock,
 * records the purchase, and posts the journals in one PostgreSQL transaction
 * so a failure cannot leave a partial inventory or financial record.
 */

// Resolve the settlement account (cash drawer or a bank account) for a method.
async function settlementAccount(method, bankAccountId, transaction = null) {
...
async function createPurchase(actor, input) {
  ...
  return getPostgres().transaction(async (transaction) => {
    ...
    if (paidPaisa > total)
      throw ApiError.badRequest('Paid amount cannot exceed the purchase total');
    ...
    if (paidPaisa > 0) {
      ...
      settle = await settlementAccount(method, bankAccount, transaction);
      await paymentService.assertSufficientFunds(settle.account, settle.ref, paidPaisa);
    }

    const when = date ? new Date(date) : new Date();
    const number = await counterService.nextDocNumber('GP', when.getFullYear(), 4, transaction);
    ...
    await GoodsPurchaseItem.bulkCreate(
      lineItems.map((item, position) => ({ ...item, position, purchaseId: purchase.id })),
    ...
    for (const p of parsed) {
      await Product.update(
        { purchasePrice: p.enteredUnitCost },
        { where: { id: p.product.id }, transaction },
      );
    }
    ...
    purchaseLines.push(journalService.line(ACCOUNT.AP, { credit: total, ref: vendorDoc.id }));
    ...
    if (paidPaisa > 0) {
      await journalService.post({
        ...
        createdBy: actor ? actor._id : null,
        lines: [
          journalService.line(ACCOUNT.AP, { debit: paidPaisa, ref: vendorDoc.id }),
    ...
    await invoiceService.createFromPurchase(purchase.id, transaction);

    // A goods-in gate pass for this purchase, the inbound counterpart of the
    // one every sale gets automatically.
    await gatePassService.createForPurchase(purchase, vendorDoc, transaction);

    return getPurchaseById(purchase.id, transaction);
  });
}
...
async function listPurchases({ vendor, from, to, ...query } = {}) {
  ...
  const [purchases, total] = await Promise.all([
    ...
    GoodsPurchase.count({ where }),
  ]);

  return { purchases, total, page, limit };
}

async function getPurchaseById(id, transaction = null) {
  const purchase = await GoodsPurchase.findByPk(id, {
    include: [
      { model: GoodsPurchaseItem, as: 'items' },
