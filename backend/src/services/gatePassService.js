...
async function persistModern(parent, items, transaction) {
  const sourceWhere = parent.stockReceipt
    ? { stockReceipt: parent.stockReceipt }
    : parent.saleReturn
      ? { saleReturn: parent.saleReturn, warehouse: parent.warehouse }
      : { sale: parent.sale, kind: parent.kind, warehouse: parent.warehouse, saleReturn: null };
  let gatePass = await GatePass.findOne({ where: sourceWhere, transaction });
  if (!gatePass) {
    const when = new Date(parent.saleDate || Date.now());
    gatePass = await GatePass.create(
      {
        ...parent,
        number: await counterService.nextDocNumber('GATE', when.getFullYear(), 6, transaction),
    ...
    await gatePass.update(parent, { transaction });
    await GatePassItem.destroy({ where: { gatePassId: gatePass.id }, transaction });
  }
  if (gatePass.status === 'PENDING') {
    await GatePassItem.bulkCreate(
...
async function createGatePassesForSale(sale, customer = null, outerTransaction = null) {
  const write = async (transaction) => {
    const products = await productsById(sale.items, transaction);
    const warehouseGroups = new Map();
    const vendorItems = [];
    for (const item of sale.items || []) {
      if (item.source === 'VENDOR' || item.vendor) vendorItems.push(item);
      else {
        const warehouse = refId(item.warehouse) || refId(sale.warehouse);
        if (!warehouseGroups.has(warehouse)) warehouseGroups.set(warehouse, []);
        warehouseGroups.get(warehouse).push(item);
      }
    }
    const links = [];
    for (const [warehouse, items] of warehouseGroups) {
      const pass = await persistModern(
        {
          sourceType: 'SALE',
          sale: refId(sale),
          kind: 'CUSTOMER',
          documentNumber: sale.number,
          partyName: customer?.name || sale.customerName || 'Walk-in',
          store: refId(sale.store),
          warehouse,
          saleDate: sale.date,
          customer: refId(customer) || refId(sale.customer),
          customerName: customer?.name || sale.customerName || 'Walk-in',
          createdBy: refId(sale.createdBy),
        },
        items.map((item) => ({
          ...item,
          ...(products.get(String(refId(item.product))) || {}).dataValues,
        })),
        transaction,
      );
      await SaleWarehouseGatePass.upsert(
        { saleId: refId(sale), warehouseId: warehouse, gatePassId: pass.id },
        { transaction },
      );
      links.push({ warehouse, gatePass: pass.id });
    }
    let vendorGatePass = null;
    if (vendorItems.length) {
      vendorGatePass = await persistModern(
        {
          sourceType: 'SALE',
          sale: refId(sale),
          kind: 'VENDOR',
    ...
    const first = links[0]?.gatePass || vendorGatePass?.id || null;
    await Sale.update(
      { gatePass: first, vendorGatePass: vendorGatePass?.id || null },
      { where: { id: refId(sale) }, transaction },
    );
    return {
      gatePass: first,
      warehouseGatePasses: links,
      vendorGatePass: vendorGatePass?.id || null,
    };
  };
  return outerTransaction ? write(outerTransaction) : getPostgres().transaction(write);
}

async function createGatePassesForReturn(saleReturn, sale, outerTransaction = null) {
  const write = async (transaction) => {
    const groups = new Map();
    for (const item of saleReturn.items || []) {
      if (item.source === 'VENDOR') continue;
      const warehouse = refId(item.warehouse) || refId(sale.warehouse);
      if (!groups.has(warehouse)) groups.set(warehouse, []);
      groups.get(warehouse).push(item);
    }
    const links = [];
      ...
      const pass = await persistModern(
        {
          ...
          customer: refId(saleReturn.customer),
          customerName: saleReturn.customerName || '',
          createdBy: refId(saleReturn.createdBy),
        },
        items,
        transaction,
      );
      await SaleReturnGatePass.upsert(
        { saleReturnId: refId(saleReturn), warehouseId: warehouse, gatePassId: pass.id },
        { transaction },
      );
      links.push({ warehouse, gatePass: pass.id });
    }
    return { warehouseGatePasses: links };
  };
  return outerTransaction ? write(outerTransaction) : getPostgres().transaction(write);
}

async function createForReceipt(receipt, outerTransaction = null) {
  const write = async (transaction) => {
    const pass = await persistModern(
      {
        sourceType: 'PURCHASE',
        stockReceipt: refId(receipt),
        kind: 'CUSTOMER',
        documentNumber: receipt.number,
        partyName: receipt.supplierName || 'Supplier',
        store: refId(receipt.store),
        warehouse: refId(receipt.warehouse),
        saleDate: receipt.date,
        vendorName: receipt.supplierName || 'Supplier',
        createdBy: refId(receipt.createdBy),
      },
      receipt.items || [],
      transaction,
    );
    await StockReceipt.update(
      { gatePass: pass.id },
      { where: { id: refId(receipt) }, transaction },
    );
    return pass;
  };
  return outerTransaction ? write(outerTransaction) : getPostgres().transaction(write);
...
async function deleteGatePass(id) {
  ...
  if (!row) throw ApiError.notFound('Gate pass not found');
  if (row.status === 'PROCESSED')
    throw ApiError.conflict('Processed gate passes cannot be deleted');
  await row.destroy();
}
